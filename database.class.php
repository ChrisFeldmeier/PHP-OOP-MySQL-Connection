...
class db implements DatabaseManager {
		...
		public static $DatabaseMonitor = true; // show sql queries and time
	...
	*/	
	public static function DatabaseMonitor()
	{
			...
			if(isset($_GET["log"]))
			{
				...
				<br />
				<div align="center">
				<table style="border: 1px solid #000; background-color:white" rules="all">
				<caption style="color:white">Queries total: '.self::$CountQueries.' - Total duration:  '.$total_queries.' ms</caption>
				<tr>
					<th>Count</th>
					<th>Query</th>
					<th width="70">Time</th>
		...
		*/
		public static function getTable($tableName, $extras = "") {
			if (!self::$db) self::connect();
			
			$tableName = self::$db->escape_string($tableName);

			return self::getObjects('SELECT * FROM `'.$tableName.'` '.$extras);
		}
		...
		* @return {bool} 
		*/		
		public static function insertTableRow($tableName, $objs = array()) {
			if(is_array($objs)) {
				$aryValues = array();
				$sql = null;
				$sql.= "INSERT INTO ".self::$db->escape_string($tableName)." (";
				foreach($objs as $key => $val) {
					$sql.= "`".strtoupper($key)."`,";
					array_push($aryValues, self::$db->escape_string($val));
				}
		...
		* @return {bool} 
		*/		
		public static function updateTableRow($tableName, $objs = array(), $keyID, $keyValue) {
			if(is_array($objs)) {
				unset ($objs[$keyID]);
				$sql = null;
				$sql.= "UPDATE ".self::$db->escape_string($tableName)." SET ";
				foreach($objs as $key => $val) {
					if(is_string($val)) {
						$sql.= "`".strtoupper($key)."` = '".$val."',";
					}
					else{
						if($val==""||empty($val)) {
							$val = "null";
